	...
	con=ConnectionProvider.getConnection();
            ...
            for(int j = 1; j <= i; j++)
            {
                ...
                if(s2 != null)
                {
                    ...
                    for(rs = st.executeQuery("select mailid,mailfrom,mailto,subject,mailcc,maildata,maildate,mailst,MAILEXCH from newcompose where mailid=" + k); rs.next();)
                    {
                        id = rs.getInt(1);
                        from = rs.getString(2);
                    ...
                    int l = st.executeUpdate("insert into newfolder values(" + k + ",'" + from + "','" + to + "','" + subj + "','" + mcc + "','" + data + "','" + s + "',"+status+",'" + mdate + "','"+at+"')");
